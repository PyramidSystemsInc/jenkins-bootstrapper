...
function deploySeleniumCluster() {
...
# function deploySonarQubeCluster() {
...
# Ensure an IAM role is provided in user input
...
# Ensure a jobs.json is provided in user input
...
# Ensure a project name is provided in user input
...
# Ensure all the required inputs were provided in user input
function ensureRequiredInputsProvided() {
...
function loadOrOverwriteAwsCredentials() {
...
# Login to Jenkins EC2 instance
function loginToJenkinsInstance() {
...
# Show help if `-h` or `--help` flags were provided, then quit 
function showHelp() {
...
function waitUntilJenkinsRunning() {
	echo -e ""
...
deployJenkinsInstance
waitUntilJenkinsRunning
copyFilesToInstance
monitorJenkinsConfiguration
changeMessageOfTheDay
loginToJenkinsInstance
echo -e ""
