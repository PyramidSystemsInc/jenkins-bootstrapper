...
# Create ECS cluster
ecs-cli up --keypair $PROJECT_NAME-jenkins-slaves --capability-iam --size 1 --instance-type t2.medium --cluster-config $PROJECT_NAME-jenkins-slaves
...
# Create ECS tasks
ecs-cli compose up --create-log-groups --cluster-config $PROJECT_NAME-jenkins-slaves
